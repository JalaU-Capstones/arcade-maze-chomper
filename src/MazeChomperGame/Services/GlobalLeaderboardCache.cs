...
public class GlobalLeaderboardCache
{
    ...
    {
        // Requirement: Linux uses ~/.cache/arcade-maze-chomper; Windows uses %LOCALAPPDATA%\\arcade-maze-chomper.
        if (OperatingSystem.IsWindows())
        {
            return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "arcade-maze-chomper");
        }

        var xdgCacheHome = Environment.GetEnvironmentVariable("XDG_CACHE_HOME");
        if (!string.IsNullOrWhiteSpace(xdgCacheHome))
        {
            return Path.Combine(xdgCacheHome, "arcade-maze-chomper");
        }

        var home = Environment.GetFolderPath(Environment.SpecialFolder.UserProfile);
        return Path.Combine(home, ".cache", "arcade-maze-chomper");
    }
