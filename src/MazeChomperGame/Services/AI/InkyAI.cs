...
public class InkyAI : IGhostAI
{
    ...
    /// <summary>
    /// Inky's behavior is complex, based on Arcade Maze Chomper's position and Blinky's position.
    /// </summary>
    public Direction GetNextMove(Ghost ghost, Pacman pacman, TileType[,] map, List<Ghost> allGhosts, bool isChaseMode, ILogger logger)
    {
        ...
        if (isChaseMode)
        {
            // Chase Mode: Target is calculated using Blinky's position and Arcade Maze Chomper's position.
            ...
            // 1. Find "pivot point" = 2 tiles ahead of Arcade Maze Chomper
            int pivotY = pacman.Y;
            int pivotX = pacman.X;
