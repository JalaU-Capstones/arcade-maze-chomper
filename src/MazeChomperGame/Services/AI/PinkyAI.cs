...
public class PinkyAI : IGhostAI
{
    ...
    /// <summary>
    /// Pinky's behavior is to ambush Arcade Maze Chomper by targeting 4 tiles ahead of Arcade Maze Chomper's direction.
    /// </summary>
    public Direction GetNextMove(Ghost ghost, Pacman pacman, TileType[,] map, List<Ghost> allGhosts, bool isChaseMode, ILogger logger)
    {
        ...
        if (isChaseMode)
        {
            // Chase Mode: Target 4 tiles ahead of Arcade Maze Chomper's direction.
            targetY = pacman.Y;
            targetX = pacman.X;
