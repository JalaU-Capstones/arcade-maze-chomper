...
public class ConsoleViewModel : ViewModelBase
{
    ...
    public void Open()
    {
        ...
        if (!_introDisplayed)
        {
            AppendMessage(new ConsoleMessage("Arcade Maze Chomper Recreation Console v1.0.0", ConsoleMessageType.System));
    ...
    private async Task ExecuteCurrentCommandAsync()
    {
        ...
        AppendMessage(new ConsoleMessage($"arcade-maze-chomper/{_profileManager.GetActiveProfile()?.Name ?? "guest"}/$ {input}", ConsoleMessageType.Input));
        CurrentCommand = string.Empty;
